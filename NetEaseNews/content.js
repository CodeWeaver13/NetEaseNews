...
{
    var headerHTML = "<div id='comment_section' class='section' aria-hidden='true'><div class='section_header'>热门跟贴</div><ul class='section_body'><a id='comment_list' href='comment://' class='no_tc'></a><li><a class='comment_entry no_tc' href='comment://'><div>查看更多跟贴</div></a></li></ul><div class='section_footer'></div></div>";
...
function buildFloors(floors)
{
    ...
    for (var i = 0,len = floors.length; i < len; i++) {
        ...
        var bigv    = f["bigv"];
        var mars    = f["mars"];
        buildSingleFloor(icon,mars,name,subname,vote,body,bigv,first);
        first = false;
    };
}

function buildSingleFloor(usericon,mars,username,subname,vote,body,bigv,first)
{
    ...
    var iconImg = "<div class='floor_userimage' mars='" + mars + "'></div>";
    if (usericon.length > 1 && mars=="false") {
        iconImg = "<div class='floor_userimage'><img src='" + usericon + "' onerror='clearImage(this)'></img></div>";
    };
    var bigvTag = "";
    if (bigv == true) bigvTag = "bigv='true'";
    titleDiv.innerHTML = voteSpan + iconImg + "<div class='floor_username' " + bigvTag + ">" + username + "</div> <div class='floor_subname'>" + subname + "</div>";
...
function rateButtonClicked(element){
    ...
    if (index == 0){
        ...
        upTimes = parseInt(upButton.lastChild.innerText);
        downTimes = parseInt(downButton.lastChild.innerText);
        
        //更新“顶”按钮数字和状态
        upButton.lastChild.innerText = upTimes+1;
        upButton.setAttribute("selected","true");
        
        //如果“踩”是选中的，也需要-1
        if (downButton.getAttribute("selected") == "true") {
            downButton.lastChild.innerText = downTimes-1;
